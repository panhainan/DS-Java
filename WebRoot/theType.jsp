...
<title><s:property value="type.typesCategory.name" />-<s:property
		value="type.name" />
</title>
			...
			<div class="smallNav" align="left">
				...
				<div style="float: left;line-height:24px;">
					&nbsp;&gt;&nbsp;<a href="index.jsp">论坛</a>&nbsp;&gt;&nbsp;<a
						href="cate_goCate.action?category.id=<s:property value="type.typesCategory.id" />"><s:property
							value="type.typesCategory.name" /> </a>&nbsp;&gt;&nbsp;
			...
			<div>
				<div class="editMenu">
					<input type="button" value="发帖" class="butt" style="width: 80px;"
						...
						onclick="window.location.href = 'type_goType.action?type.id=<s:property value="type.id" />'"
					...
					<s:iterator value="pageBean">
						...
						<s:elseif test="%{totalPages == 2}">
							<s:if test="currentPage==1">
								...
								<button disabled="disabled">1</button>
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=2"><button>2</button>
								</a>
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
							...
							<s:elseif test="currentPage==2">
								...
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=1"><button>1</button>
								</a>
						...
						<s:elseif test="%{totalPages == 3}">
							<s:if test="currentPage==1">
								...
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=3"><button>3</button>
								</a>
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
										style="width: 80px;">下一页</button> </a>
						...
						<s:elseif test="%{totalPages > 3}">
							<s:if test="%{currentPage==1}">
								...
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=<s:property value="%{totalPages}" />"><button>
							...
							<s:elseif test="%{currentPage==totalPages}">
								...
								<a><button disabled="disabled">...</button> </a>
								<a
									href="type_goType.action?type.id=<s:property value="type.id" />&&nowPage=<s:property value="%{totalPages-1}" />"><button>
				...
				<script type="text/javascript">
					function goPage() {
						...
						if (selectedPage != 0 && selectedPage != currentPage) {
							document.goPageForm.action = "type_goType.action?type.id=<s:property value='type.id' />&&nowPage="
									+ selectedPage;
							goPageForm.submit();
						}
						...
						<tr>
							<td align="left"><div
									...
									&nbsp;&nbsp;<font style="font-size: 13px;color: #4C9ED9"><s:if
										...
										<s:else></s:else>[<s:if test="#topic.status==0">未结帖</s:if> <s:else>已结帖</s:else>]</font><a
							...
							<td style="font-size: 12px;" width="100px;" align="right"><s:date
									name="#topic.topicTime" format="MM.dd HH:mm"></s:date>
							</td>
						</tr>
					</s:iterator>
				</table>
