	...
	// Pass this if window is not defined yet
		...
		(
				...
				function(window, noGlobal) {
					...
					// Make sure we trim BOM and NBSP (here's looking at you,
					// Safari 5.0 and IE)
					rtrim = /^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,
					...
					jQuery
							.extend({
								...
								// Use native String.trim function wherever
								// possible
								trim : trim && !trim.call("\uFEFF\xA0") ? function(
										text) {
									return text == null ? "" : trim.call(text);
								}
										:

										// Otherwise use our own trimming
										// functionality
										function(text) {
											return text == null ? ""
													: (text + "").replace(
					...
					(function(window) {
						...
						// then attribute selectors/non-parenthetical
						// expressions,
						// then anything else
						// These preferences are here to reduce the number of
						// selectors
						...
						function select(selector, context, results, seed) {
							...
							if (!seed) {
								// Try to minimize operations if there is only
								// one group
								if (match.length === 1) {

									// Take a shortcut and set the context if
									// the root selector is an ID
									...
									if (tokens.length > 2
										...
										selector = selector.slice(tokens
												.shift().value.length);
									}

									// Fetch a seed set for right-to-left
									// matching
									i = matchExpr["needsContext"]
											.test(selector) ? 0 : tokens.length;
									while (i--) {
										token = tokens[i];
					...
					jQuery(function() {
						// We need to execute this one support test ASAP because
						// we need to know
						// if body.style.zoom needs to be set.

						var container, div, body = document
					...
					(function() {
						var fragment = document.createDocumentFragment(), div = document
								.createElement("div"), input = document
								.createElement("input");
						...
						special : {
							...
							beforeunload : {
								postDispatch : function(event) {
									...
									// Even when returnValue equals to undefined
									// Firefox will still show alert
						...
						if (src && src.type) {
							...
							this.isDefaultPrevented = src.defaultPrevented
									...
									src.getPreventDefault
											&& src.getPreventDefault()) ? returnTrue
									: returnFalse;
					...
					(function() {
						var a, shrinkWrapBlocksVal, div = document
								.createElement("div"), divReset = "-webkit-box-sizing:content-box;-moz-box-sizing:content-box;box-sizing:content-box;"
					...
					(function() {
						var a, reliableHiddenOffsetsVal, boxSizingVal, boxSizingReliableVal, pixelPositionVal, reliableMarginRightVal, div = document
								.createElement("div"), containerStyles = "border:0;width:0;height:0;position:absolute;top:0;left:-9999px", divReset = "-webkit-box-sizing:content-box;-moz-box-sizing:content-box;box-sizing:content-box;"
								+ "display:block;padding:0;margin:0;border:0";
								...
								.extend(
										support,
										{
											reliableHiddenOffsets : function() {
												...
												var container, tds, isSupported, div = document
														.createElement("div"), body = document
														.getElementsByTagName("body")[0];

												if (!body) {
													// Return for frameset docs
												...
												// Support: IE8
												// Check if table cells still
												// have offsetWidth/Height when
												// they are set
												// to display:none and there are
												// still other visible table
												// cells in a
												// table row; if so,
												// offsetWidth/Height are not
												// reliable for use when
												// determining if an element has
												// been hidden directly using
												// display:none (it is still
												// safe to use offsets if a
												// parent element is
												// hidden; don safety goggles
												// and see bug #4512 for more
												// information).
												...
												// Support: IE8
												// Check if empty table cells
												// still have offsetWidth/Height
												reliableHiddenOffsetsVal = isSupported
														&& (tds[0].offsetHeight === 0);

												body.removeChild(container);

												// Null elements to avoid leaks
												// in IE.
												div = body = null;
											...
											reliableMarginRight : function() {
												...
												if (reliableMarginRightVal == null
													...
													body = document
															.getElementsByTagName("body")[0];
													if (!body) {
														// Test fired too early
														// or in an unsupported
														// environment, exit.
														return;
													}
						...
						function computeStyleTests() {
							...
							// Workaround failing boxSizing test due to
							// offsetWidth returning wrong value
							// with some non-1 values of body zoom, ticket
							// #13543
							...
							// Use window.getComputedStyle because jsdom on
							// node.js will break without it.
							if (window.getComputedStyle) {
					...
					function getWidthOrHeight(elem, name, extra) {
						...
						var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth
								: elem.offsetHeight, styles = getStyles(elem), isBorderBox = support
								.boxSizing()
									...
									if (value !== undefined) {
										...
										if (!hooks
											...
											try {
												...
												// Setting style to blank string
												// required to delete "style: x
												// !important;"
												style[name] = "";
												style[name] = value;
											} catch (e) {
					...
					function defaultPrefilter(elem, props, opts) {
						...
						var prop, value, toggle, tween, hooks, oldfire, display, dDisplay, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
					...
					(function() {
						var a, input, select, opt, div = document
								.createElement("div");
