					...
					<td
						...
						<div style="width: 400px;text-align: left;float: left;">
							...
							<font style="font-size: 12px;color: #817E7E;">&nbsp;状态：</font> <font
								...
								已结帖
							</s:else> </font> <font style="font-size: 12px;color: #817E7E;">[问题积分: <s:property
									value="#comment.commentsTopic.integral" />]&nbsp;[评论量: <s:property
						...
						<div style="width: 300px;text-align: right;float: left;">
							<font style="font-size: 14px;color: #817E7E;">评论时间:<s:date
									name="#comment.commentTime" format="yyyy-MM-dd HH:mm"></s:date>
							</font>
						</div>
					</td>
				</tr>
						...
						<div
							...
							&nbsp;我评论了<a
								href="user_GoUser.action?user.id=<s:property
										value="#comment.commentsTopic.topicsUser.id" />"
								target="_top" style="font-size: 15px;">“<s:property
									value="#comment.commentsTopic.topicsUser.nickname" />”</a>的帖子<a
								style="font-size: 16px;font-weight: bold;"
								href="topic_goTopic.action?topic.id=<s:property
								value="#comment.commentsTopic.id" />"
								target="_top" class="topicTitle">“<s:property
									value="#comment.commentsTopic.title" />”</a> [<a
								href="cate_goCate.action?category.id=<s:property value="#comment.commentsTopic.topicsType.typesCategory.id" />"
								target="_top" style="font-size: 13px;"><s:property
									value="#comment.commentsTopic.topicsType.typesCategory.name" />
							</a>-<a
								href="type_goType.action?type.id=<s:property
								value="#comment.commentsTopic.topicsType.id" />"
								target="_top" style="font-size: 13px;"><s:property
									value="#comment.commentsTopic.topicsType.name" /> </a>]
							<s:if test="#comment.commentsTopic.niceTopic==1">[<font
									color="red">精品</font>]</s:if>
							<s:else></s:else>
						</div>
					</td>
				</tr>
			...
			<s:iterator value="pageBean">
				...
				<s:elseif test="%{totalPages == 2}">
					<s:if test="currentPage==1">
						<button disabled="disabled" style="width: 80px;">上一页</button>
						<button disabled="disabled">1</button>
						<a href="user_GetComments.action?nowPage=2"><button>2</button>
						</a>
						<a
							href="user_GetComments.action?nowPage=<s:property value="%{currentPage+1}"/>"><button
					...
					<s:elseif test="currentPage==2">
						...
						<a href="user_GetComments.action?nowPage=1"><button>1</button>
						</a>
				...
				<s:elseif test="%{totalPages == 3}">
					<s:if test="currentPage==1">
						...
						<a href="user_GetComments.action?nowPage=3"><button>3</button>
						</a>
						<a
							href="user_GetComments.action?nowPage=<s:property value="%{currentPage+1}"/>"><button
								style="width: 80px;">下一页</button> </a>
				...
				<s:elseif test="%{totalPages > 3}">
					<s:if test="%{currentPage==1}">
						...
						<a
							href="user_GetComments.action?nowPage=<s:property value="%{totalPages}" />"><button>
					...
					<s:elseif test="%{currentPage==totalPages}">
						...
						<a><button disabled="disabled">...</button> </a>
						<a
							href="user_GetComments.action?nowPage=<s:property value="%{totalPages-1}" />"><button>
		...
		<script type="text/javascript">
			function goPage() {
				...
				if (selectedPage != 0 && selectedPage != currentPage) {
					document.goPageForm.action = "user_GetComments.action?nowPage="
							+ selectedPage;
					goPageForm.submit();
				}
