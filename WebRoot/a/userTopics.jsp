...
<title>管理“<s:property value="user.nickname"/>”的帖子</title>
...
<script type="text/javascript">
	function goPage() {
		...
		if (selectedPage != 0 && selectedPage != currentPage) {
			document.goPageForm.action = "user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage="
					+ selectedPage;
			goPageForm.submit();
		}
...
<body>
	<div class="body">
		...
		<div class="right">
			<div align="center" style="font-size: 24px;margin-top: 10px;">管理“<s:property value="user.nickname"/>”的帖子<font style="font-size:14px; ">(共有<s:property value="pageBean.allRecords"/>帖子)</font></div>
				...
				<s:iterator value="pageBean">
					...
					<s:elseif test="%{totalPages == 2}">
						<s:if test="currentPage==1">
							<button disabled="disabled" style="width: 80px;">上一页</button>
							<button disabled="disabled">1</button>
							<a href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=2"><button>2</button>
							</a>
							<a
								href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
						...
						<s:elseif test="currentPage==2">
							...
							<a href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=1"><button>1</button>
							</a>
					...
					<s:elseif test="%{totalPages == 3}">
						<s:if test="currentPage==1">
							...
							<a href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=3"><button>3</button>
							</a>
							<a
								href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
									style="width: 80px;">下一页</button> </a>
					...
					<s:elseif test="%{totalPages > 3}">
						<s:if test="%{currentPage==1}">
							...
							<a
								href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{totalPages}" />"><button>
						...
						<s:elseif test="%{currentPage==totalPages}">
							<a
								href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{currentPage-1}"/>"><button
									style="width: 80px;">上一页</button> </a>
							<a href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=1"><button>1</button>
							</a>
							<a><button disabled="disabled">...</button> </a>
							<a
								href="user_ManageUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{totalPages-1}" />"><button>
			...
			<div class="pageGo" align="right">
				<form action="user_ManageUserAllTopics.action?user_ManageUserAllTopics.action?" method="post"
					name="goPageForm">
				...
				<s:iterator value="listTopic" var="topic">
					<tr style="height: 26px;">
						...
						<td style="font-size: 12px;"><s:property
								...
								value="#topic.topicsType.typesCategory.name" />
						</td>
						<td><s:property value="#topic.countComment" /></td>
						<td><s:property value="#topic.integral" /></td>
					...
					<%-- 	<td style="color: blue;"><s:if test="#topic.niceTopic==0">
							...
							</s:else>&nbsp;&nbsp;&nbsp;&nbsp;<a style="color: silver;">删除</a></td> --%>
					</tr>
