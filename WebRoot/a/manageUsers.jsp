	...
	<div class="body">
		...
		<div class="right">
			<div align="center" style="font-size: 24px;margin-top: 10px;">
				用 户 管 理<font style="font-size: 14px;">(共有<s:property
				...
				<s:iterator value="pageBean">
					...
					<s:elseif test="%{totalPages == 2}">
						<s:if test="currentPage==1">
							<button disabled="disabled" style="width: 80px;">上一页</button>
							<button disabled="disabled">1</button>
							<a href="user_ManageAll.action?nowPage=2"><button>2</button>
							</a>
							<a
								href="user_ManageAll.action?nowPage=<s:property value="%{currentPage+1}"/>"><button
						...
						<s:elseif test="currentPage==2">
							...
							<a href="user_ManageAll.action?nowPage=1"><button>1</button>
							</a>
					...
					<s:elseif test="%{totalPages == 3}">
						<s:if test="currentPage==1">
							...
							<a href="user_ManageAll.action?nowPage=3"><button>3</button>
							</a>
							<a
								href="user_ManageAll.action?nowPage=<s:property value="%{currentPage+1}"/>"><button
									style="width: 80px;">下一页</button> </a>
					...
					<s:elseif test="%{totalPages > 3}">
						<s:if test="%{currentPage==1}">
							...
							<a
								href="user_ManageAll.action?nowPage=<s:property value="%{totalPages}" />"><button>
						...
						<s:elseif test="%{currentPage==totalPages}">
							...
							<a><button disabled="disabled">...</button> </a>
							<a
								href="user_ManageAll.action?nowPage=<s:property value="%{totalPages-1}" />"><button>
			...
			<table class="userStyle" cellspacing="0">
				...
				<s:iterator value="listUser" var="us">
					<tr style="height: 26px;">
						<td style="border-left: 1px solid silver;"><s:property
								value="#us.username" />
						</td>
						<td><s:property value="#us.nickname" />
						</td>
						<td><s:if test="#us.roleId==0">普通用户</s:if> <s:elseif
								test="#us.roleId==6">普通管理员</s:elseif> <s:elseif
								test="#us.roleId==16">高级管理员</s:elseif>
						</td>
						<td><s:if test="#us.status==0">可用状态</s:if> <s:else>禁用状态</s:else>
						</td>
						<td><s:property value="#us.usersGrade.id" />:<s:property
								value="#us.usersGrade.honor" />
						</td>
						<td><s:property value="#us.integral" />
						</td>
						<td><s:property value="#us.gradeIntegral" />
						</td>
						<td style="font-size: 12px;color: blue;"><a
							href="user_ManageGoUser.action?user.id=<s:property value="#us.id" />">查看详细&gt;&gt;</a>
						</td>
						<td style="color: blue;"><s:if test="#us.roleId!=16">
								<s:if test="#us.status==1">
									<a style="color: red"
										href="user_ManageUnDelete.action?user.id=<s:property value="#us.id" />"
										onclick="unDeleteUser();">解禁</a>
								</s:if>
								<s:else>
									<a
										href="user_ManageDelete.action?user.id=<s:property value="#us.id" />"
										onclick="deleteUser(<s:property value="#us.id" />);">禁用</a>
								</s:else>
							</s:if> <s:else>
