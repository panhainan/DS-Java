...
<title><s:property value="user.nickname" />的帖子</title>
			...
			<div>
				<div class="editMenu">
					<input type="button" value="发帖" class="butt" style="width: 80px;"
						...
						onclick="window.location.href = 'user_GoUserAllTopics.action?user.id=<s:property value="user.id" />'"
					...
					<s:iterator value="pageBean">
						...
						<s:elseif test="%{totalPages == 2}">
							<s:if test="currentPage==1">
								...
								<button disabled="disabled">1</button>
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=2"><button>2</button>
								</a>
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
							...
							<s:elseif test="currentPage==2">
								...
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=1"><button>1</button>
								</a>
						...
						<s:elseif test="%{totalPages == 3}">
							<s:if test="currentPage==1">
								...
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=3"><button>3</button>
								</a>
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{currentPage+1}"/>"><button
										style="width: 80px;">下一页</button> </a>
						...
						<s:elseif test="%{totalPages > 3}">
							<s:if test="%{currentPage==1}">
								...
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{totalPages}" />"><button>
							...
							<s:elseif test="%{currentPage==totalPages}">
								...
								<a><button disabled="disabled">...</button> </a>
								<a
									href="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />&&nowPage=<s:property value="%{totalPages-1}" />"><button>
				...
				<script type="text/javascript">
					function goPage() {
						...
						if (selectedPage != 0 && selectedPage != currentPage) {
							document.goPageForm.action = "user_GoUserAllTopics.action?user.id=<s:property value='user.id' />&&nowPage="
									+ selectedPage;
							goPageForm.submit();
						}
				...
				<div class="pageGo" align="right">
					<form
						action="user_GoUserAllTopics.action?user.id=<s:property value="user.id" />"
						method="post" name="goPageForm">
			...
			<div class="titleStyle" align="left">
				<table border="0" style="margin: 0;		font-size: 12px;">
					<tr style="height: 30px;line-height: 30px;">
						...
						<td style="width:520px" align="left">标题</td>
						<td style="width:60px" align="center">精品</td>
				...
				<table align="left" class="listTopicStyle">
					<s:iterator value="listTopic" id="topic">
						<tr>
							...
							<td align="left"><div
									style="width:520px; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;float: left;">
							...
							<td align="center" width="60px;"><s:if
									test="#topic.niceTopic==1">是</s:if> <s:else>
										否</s:else></td>
							...
							<td style="font-size: 12px;" width="110px;" align="right"><s:date
									name="#topic.topicTime" format="yyyy-MM-dd HH:mm"></s:date></td>
						</tr>
					</s:iterator>
				</table>
			</div>
